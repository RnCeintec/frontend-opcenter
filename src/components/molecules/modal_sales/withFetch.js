...
const withFetch = (Component) => (props) => {
  ...
  const [atencionDataById, setAtencionDataById] = useState(null);
  ...
  const fetchAtencion = useFetch('diotrias');


  const getAtencionDataById = async () => {
    // const body = { descripcion: searchClient };
    try {
      // const atencionId = 
      const [result, status] = await fetchAtencion.getById(atencionId);
      debugger
      if (status === 200) setAtencionDataById(result);
      
      else errorAlert('Error al Conectar, vuelta a intentarlo');
    } catch (error) {
      console.log(error);
      errorAlert('Error al Conectar, vuelta a intentarlo');
      setAtencionDataById({ result: [] });
    }
  }
  ...
  return (
    <Component
      ...
      atencionDataById={atencionDataById?.result || {}}
