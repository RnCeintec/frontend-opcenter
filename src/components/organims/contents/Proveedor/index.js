...
const Proveedor = ({
  isLoadingSearchProveedor,
  setIsLoadingSearchProveedor,
  categoryId,
  setCategoryId,
  ...
  useEffect(() => {
    if (typeOfPanel === 'Editar') {
      setValue('ruc', proveedorByIdData.ruc || '');
      setValue('razonsocial', proveedorByIdData.razonsocial || '');
      setValue('direccion', proveedorByIdData.direccion || '');
      setValue('telefono', proveedorByIdData.telefono || '');
      setValue('contacto', proveedorByIdData.contacto || '');
  ...
  const createProveedor = async (data) => {
    ...
    const [result, status] = await fetchProveedor.post(dataToAdd);
    if (status !== 200) {
      ...
      clearInputs();
      successAlert('Proveedor Creado Correctamente');
    }
    // }
    //   else {
    //   setIsLoaderSubmit(false);
    // }
    await setProveedorMutate(Date.now());
  };

  // EDIT ADMIN
  const editProveedor = async (data) => {
    ...
    try {
      const [result, status] = await fetchProveedor.put(idProveedor, dataToAdd);
      if (status !== 200) {
        errorAlert(result.message);
        setIsLoaderSubmit(false);
      } else {
        ...
        setIsRowBlocked(false);
        clearInputs();
        successAlert('Proveedor Actualizado Correctamente');
      }
    } catch (error) {
      console.log(error);
    } finally {
      setProveedorMutate(Date.now);
      scrollUp();
    }
  };
  ...
  const handleDeleteProveedor = async () => {
    ...
    const id = idToDelete;
    try {
      const [result, status] = await fetchProveedor.del(id);
      if (status !== 200) {
            ...
            <ModalTwoOptions
              titleModal={(
                <>
                  ...
                  <p>el Proveedor:</p>
                  <p className="pb-2">{`${proveedorData?.filter((item) => item.id === idToDelete)[0]?.razonsocial}?`}</p>
                </>
              )}
              ...
              onClickSecondOption={() => handleDeleteProveedor()}
            ...
            <div className="w-full bg-white rounded-xl shadow sm:mb-0">
                ...
                <div className="flex">
                  ...
                  >
                    <input
                      ...
                      placeholder="Buscar"
                      onChange={(e) => {
                        setIsLoadingSearchProveedor(true);
                        setTimeout(() => {
              ...
              <div className="p-4 overflow-auto max-h-96">
                <table id="tableAdmin" className="table-auto text-center text-gray-700 w-full">
                  <thead>
                    <tr className="border-l border-gray-400 bg-primary opacity-90 text-white">
                      ...
                      <th className="border-r border-b border-gray-200 px-6 py-2 font-medium whitespace-nowrap">Proveedor</th>
                      <th className="border-r border-b border-gray-200 px-2 py-2 font-medium">RUC</th>
                  ...
                  {Array.isArray(proveedorData) && proveedorData[0] !== 'loading'
                    && (
                      <tbody>
                        {proveedorData.map((item) => (
                          <tr key={item.id} onClick={() => !isRowBlocked && setRowSelected(item.id)} className={`text-sm rowTable ${rowSelected === item.id && 'rowTableAdminSelected'}`}>
                            ...
                            <td className="border border-gray-350 px-2 py-2 font-medium capitalize text-left">{item.razonsocial || ''}</td>
                            <td className="border border-gray-350 px-2 py-2 font-medium capitalize">{item.ruc || ''}</td>
                ...
                <div>
                  ...
                  {proveedorData[0] === 'loading'
                    ? (
                      <div className="flex items-center justify-center py-8">
                        <IconSpiner dark medium />
                      </div>
                    )
                    : proveedorData.length === 0 || !Array.isArray(proveedorData)
                      ? (
                    ...
                    <>
                      <form className="flex flex-col w-full -mt-2 relative" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <FormTextInput inputName="direccion"
                          ...
                          register={register} errors={errors}
                        />
                        <FormTextInput inputName="contacto"
                          title="Persona de Contacto"
                          icon={<IconObservation />}
                          placeholder="Ingresar Persona de Contacto (opcional)"
                          register={register} errors={errors}
                        />
                        <FormTextInput inputName="celular"
                          title="Celular"
                          icon={<IconPhone />}
                          type="money"
                          placeholder="Ingresar N° Telefono (opcional)"
                        ...
                        />
                          ...
                          title="Comentario"
                          icon={<IconObservation />}
                          placeholder="Ingresar algún comentario (opcional)"
                          register={register} errors={errors}
                        />
...
Proveedor.propTypes = {
  isLoadingSearchProveedor: PropTypes.bool.isRequired,
  setIsLoadingSearchProveedor: PropTypes.func.isRequired,
  ...
  setProveedorMutate: PropTypes.func.isRequired,
  ...
  setSearchProveedor: PropTypes.func.isRequired,
  searchProveedor: PropTypes.string.isRequired,
  idProveedor: PropTypes.number.isRequired,
  setIdProveedor: PropTypes.func.isRequired,
  proveedorByIdData: PropTypes.oneOfType([
    PropTypes.arrayOf(PropTypes.string),
    PropTypes.arrayOf(PropTypes.shape()),
  ]).isRequired,
...
export default withFetch(Proveedor);
