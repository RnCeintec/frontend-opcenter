...
import {
  IconSalesSolid, IconSearch, IconSpiner, IconClose, IconCategory, IconSelect, IconCheck, IconWhatsDoc, IconNewClient, IconFilter,
...
const Sales = ({
  productsData, setSearchProduct,
  clientsData, setSearchClient,
  setClientMutate, clientDataById,
  ...
  // PRODUCTS
  const [isOpenSearchListProducts, setIsOpenSearchListProducts] = useState(false);
  const [isOpenSearchByCategoryModal, setIsOpenSearchByCategoryModal] = useState(false);
  ...
  const handleChangePrice = (e, setter, setterOperation, setValueOne, setValueTwo) => {
    ...
    if (buttonPressed === 'ticket') {
      ...
      setData(
        formData?.client
          ? formData
          : {
            ...formData, client: 'Cliente Universal', documentType: 'DNI', document: '12345678', address: '---',
          },
      );
  ...
  const sendByWhatsapp = (whatsappData, phone, date) => {
    ...
    let whatsAPI = `https://api.whatsapp.com/send?phone=+51${phone}&text=*🛒FORMATO%20DE%20PEDIDO*%0A%0A▪NOMBRE%20:%20${(whatsappData.client).split('&').join('%26')}%0A▪CELULAR%20:%20${phone}%0A▪DIRECCION%20:%20${whatsappData.clientAddress}%0A▪FECHA%20:%20${dayjs(date).format('DD-MM-YYYY')}%0A▪HORA%20:%20${dayjs(date).format('hh:mm a')}%0A▪DETALLE%20VENTA%20:%0A${whatsDetalle}%0A▪TIPO%20DE%20PAGO%20:%20${whatsappData.payType}✅%0A▫TOTAL%20:%20S/.${(+whatsappData.amount).toFixed(2)}%0A------------------------------------------------------%0A▪PUNTO%20VENTA%20:%20*${localData.businessName.split('&').join('%26')}*`;
  ...
  const saveSale = async () => {
    ...
    const total = cartLocal.reduce((acc, { cant, price }) => acc + (cant * price), 0);
    let productToAdd = [];
    ...
    const dataToSend = {
      ...
      tipo_pago: isTwoPaysChecked ? refPayMethodTwo.current.value : refPayMethodOne.current.value,
    ...
    try {
      ...
      if (status === 200) {
        ...
        const dataToPrint = {
          ...
          payType: isTwoPaysChecked ? `Efectivo y ${refPayMethodTwo.current.value}` : refPayMethodOne.current.value,
          amount: total,
        };
        dispatch({ type: 'SET_SALE_DATA', payload: dataToPrint });
        localStorage.setItem('lastSale', JSON.stringify(dataToPrint));
        if (isWhatsappChecked) sendByWhatsapp(dataToPrint, data.phone, result.fecha_creacion);
  ...
  useEffect(() => {
    ...
    if ('cart' in localStorage) {
      ...
      totalPriceProducts = cartLocal.reduce((acc, { cant, price }) => acc + (cant * price), 0);
    }
  ...
  return (
    <>
      {isOpenSearchByCategoryModal
        && (
          <Portal>
            <SearchByCategory setIsOpenSearchByCategoryModal={setIsOpenSearchByCategoryModal} />
          </Portal>
        )}
      {isOpenNewClientModal
        && (
          <Portal>
            <NewClient setIsOpenNewClientModal={setIsOpenNewClientModal} setClientMutate={setClientMutate} setClientId={setClientId} />
          </Portal>
        )}
      {isOpenModalFinally
        && (
      ...
      <div className="flex flex-col items-center bg-bg-blue w-full min-h-screen box-border">
        ...
        <div className="flex flex-col w-full h-full p-4 ">
          <form onSubmit={handleSubmit(onSubmit)} className="shadow-md rounded-xl">
            {/* WELCOME */}
            <div className="w-full px-3 sm:px-4 flex justify-center items-center bg-white rounded-t-xl border-b relative">
              <p className="sm:text-xl text-left text-gray-600 py-1.5 sm:py-2 truncate">Formulario de Ventas</p>
            </div>
            <div className="flex flex-col md:flex-row p-4 bg-white rounded-b-xl">
              <div className="flex flex-col w-full md:w-3/5 ">
                <div className=" mb-1">
                  ...
                  <p className="font-semibold text-gray-600 -mt-3 ml-3 mb-1 sm:font-semibold ">Producto</p>
                  <div className="flex space-x-2 mb-2">
                    <label htmlFor="product" className="w-full relative flex flex-col justify-center ">
                      <div className="flex items-center">
                        <input
                          ...
                          className={`${isOpenSearchListProducts ? 'rounded-b-none' : 'rounded-b-xl'} bg-white pl-4 pr-8 pt-1 pb-1.5 rounded-t-xl w-full ring-blue-400 ring-1 focus:ring-2 focus:outline-none text-gray-800`}
                        />
                                ...
                                : productsData.length === 0 || !Array.isArray(productsData)
                                  ...
                                  : productsData.map((item, i) => (
                                    <button
                                      ...
                                      className={`${(productsData.length - 1) !== i ? 'border-b' : ''} pl-2 p-1 flex flex-wrap justify-start w-full hover:bg-blue-500 hover:text-white text-sm`}
                                    >
                    ...
                    <button type="button" onClick={() => setIsOpenSearchByCategoryModal(true)} className="flex items-center justify-center rounded-full py-1 px-2.5 text-sm bg-primary hover:opacity-90 hover:ring ring-gray-200 text-white tap-highline-none">
                      <p className="mr-1.5 hidden sm:flex md:hidden lg:flex">Categorias</p>
                  ...
                  <div className="rounded-t-xl ring-blue-400 ring-1 p-1.5 pb-2.5 max-h-screen flex flex-col space-y-1.5 overflow-y-auto bg-white">
                    {cartProducts.length === 0 || !Array.isArray(cartProducts)
                      ? (
                        <div className="flex justify-center items-center text-gray-500">
                          <h4 className="flex my-5 text-lg italic">A la Espera de Productos!</h4>
                        </div>
                      )
                  ...
                  <div className="flex justify-between rounded-b-xl ring-1 ring-blue-400 -mt-0.5 bg-white px-2.5 py-1.5 text-sm">
                    ...
                    <div className="flex">
                      ...
                      <p className="font-semibold mr-1">{parseFloat(priceProducts).toFixed(2)}</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col w-full md:w-2/5 md:-mt-1 md:pl-4">
                <label htmlFor="client" className="w-full relative mb-2 flex flex-col justify-center ">
                  <div className="flex text-sm">
                    <p className="font-semibold text-gray-600 ml-2 pl-1  sm:font-semibold ">Buscar Cliente</p>
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="flex items-center w-full relative">
                      <input
                        ...
                        className={`${errors?.client ? 'ring-red-500' : 'ring-blue-400'} ${isOpenSearchListClients ? 'rounded-t-xl' : 'rounded-xl'} bg-white pl-3 pr-3 py-0.5 text-sm  w-full ring-1 focus:ring-2 focus:outline-none text-gray-800`}
                      />
                              ...
                              : clientsData.length === 0 || !Array.isArray(clientsData)
                                ...
                                : clientsData.map((item, i) => (
                                  <button
                                    key={item.id}
                                    type="button"
                                    onClick={() => (clientId === item.id ? handleSelectClient() : setClientId(item.id))}
                                    className={`${(clientsData.length - 1) !== i ? 'border-b' : ''} pl-2 py-2 p-1 flex flex-wrap justify-start w-full hover:bg-blue-500 hover:text-white text-xs sm:text-sm md:text-xs`}
                                  >
                    ...
                    <button type="button" onClick={() => setIsOpenNewClientModal(true)} className="flex rounded-full py-1 px-2.5 text-sm bg-primary hover:opacity-90 hover:ring ring-gray-200 text-white tap-highline-none">
                      <p className="mr-1.5 hidden sm:flex md:hidden lg:flex">Nuevo</p>
                      <span><IconNewClient /></span>
                    </button>
                  </div>
                </label>
                <span className={`${errors?.client || errors?.documentType || errors?.document || errors?.address ? '' : 'hidden'} pl-2 text-xs text-red-500 mb-0.5 -mt-1.5`}>
                  {errors?.client?.message || errors?.documentType?.message || errors?.document?.message || errors?.address?.message}
                </span>
                <input
                  readOnly
                  type="text"
                  ...
                  {...register(
                    'client',
                    {
                      ...
                      validate: {
                        minBoleta: (e) => (buttonPressed === 'boleta' ? priceProducts > 699 ? e !== '' : true : true) || 'Venta con Boleta mayor a S/700 requiere Cliente',
                      },
                    },
                  )}
                />
                <input
                  ...
                  {...register(
                    'documentType',
                    {
                      ...
                      validate: {
                        minBoleta: (e) => (buttonPressed === 'boleta' ? priceProducts > 699 ? e !== '' : true : true) || 'Error con Tipo de Documento en Venta mayor a S/700',
                      },
                    },
                  )}
                />
                <input
                  ...
                  {...register(
                    'document',
                    {
                      ...
                      validate: {
                        minBoleta: (e) => (buttonPressed === 'boleta' ? priceProducts > 699 ? e !== '' : true : true) || 'Error con N° de Documento en Venta mayor a S/700',
                      },
                    },
                  )}
                />
                <input
                  ...
                  {...register(
                    'address',
                    {
                      ...
                      validate: {
                        minBoleta: (e) => (buttonPressed === 'boleta' ? priceProducts > 699 ? e !== '' : true : true) || 'Error en la Dirección en Venta mayor a S/700',
                      },
                    },
                  )}
                />
                <div className="pl-3 py-2 pr-2 ring-1 ring-blue-400 rounded-2xl">
                  <div className="flex flex-wrap text-sm relative">
                    <p className="font-semibold text-primary sm:font-semibold">Cliente:</p>
                    <p className="font-semibold ml-2 text-gray-600 capitalize">{`${(watch('client') && watch('client').toLowerCase()) || '-'}`}</p>
                  </div>
                  <div className="flex text-sm relative">
                    <p className="font-semibold text-primary sm:font-semibold">Docum.:</p>
                    <div className="flex text-gray-600 font-semibold ml-2">
                      <p className="">{`${(watch('documentType') === 'dni' && 'DNI') || (watch('documentType') === 'ruc' && 'RUC') || (watch('documentType') === 'carnet' && 'Carnet Extr.') || ''}`}</p>
                  ...
                  <div className="flex flex-wrap text-sm">
                    <p className="font-semibold text-primary sm:font-semibold">Dirección:</p>
                    <p className="font-semibold ml-2 text-gray-600 capitalize">{`${(watch('address') && watch('address').toLowerCase()) || '-'}`}</p>
                  </div>
                </div>

                <div className={`flex items-start space-x-2 mt-2 ${errors?.phone ? '' : 'mb-1'}`}>
                  <div className="w-1/2">
                    <div htmlFor="phone" className="flex items-center text-sm">
                      <p className=" font-semibold text-gray-700 ml-2 pl-1 sm:font-semibold ">N° Teléfono</p>
                      <p className="text-red-600 pr-1">{isWhatsappChecked ? '*' : ''}</p>
                      <p className="text-gray-400 text-xs">{!isWhatsappChecked ? '(opcional)' : ''}</p>
                    </div>
                    <label htmlFor="phone" className="w-full relative flex flex-col justify-center ">
                      <input
                        ...
                        type="text"
                        id="phone"
                        className={`${errors?.phone ? 'ring-red-500' : 'ring-blue-400'} bg-white pl-3 pr-3 py-0.5 text-sm rounded-xl w-full ring-1 focus:ring-2 focus:outline-none text-gray-800`}
                        {...register(
                          'phone',
                          {
                            required: {
                            ...
                            validate: {
                              length: (e) => (isWhatsappChecked ? (`${e}`).length === 9 : true) || 'Debe tener 9 digitos',
                            },
                          },
                        )}
                      />
                  ...
                  <div className="w-1/2">
                    ...
                    <label htmlFor="checkWhatsapp" className="relative flex items-center space-x-3 mt-2.5 mb-1 select-none w-full">
                      <div className={`${isWhatsappChecked ? '' : ' '} flex items-center pl-2.5 pr-2.5 py-0.5 w-full bg-white text-gray-800 rounded-full focus:outline-none`}>
                        <input
                          type="checkbox"
                          id="checkWhatsapp"
                          ...
                          className={`${isWhatsappChecked ? 'border-primary' : 'md:hover:border-primary border-gray-400'} relative appearance-none h-4 w-4 checked:bg-primary border rounded-full focus:outline-none`}
                        />
                        <span className="flex items-center ml-0.5 w-3 h-3 text-white absolute"><IconCheck /></span>
                        <div className="flex items-center ml-1">
                          <span className=""><IconWhatsDoc black={!isWhatsappChecked} /></span>
                          <div className={`${isWhatsappChecked ? 'text-gray-700' : 'text-gray-500'} ml-1 text-sm whitespace-nowrap`}>
                ...
                <div className="flex items-start">
                  {/* FIRST METHOD */}
                  <label htmlFor="checkTwoPays" className="relative flex items-center space-x-3 mb-1 select-none">
                    <div className={`${isTwoPaysChecked ? 'ring-blue-400' : ' '} group flex items-center pl-2.5 pr-2.5 py-0.5 w-full ring-0 focus:ring-0 md:hover:ring-primary bg-white ring-gray-400 text-gray-800 rounded-full focus:outline-none`}>
                      <input
                        type="checkbox"
                        id="checkTwoPays"
                        ...
                        className={`${isTwoPaysChecked ? 'border-primary' : 'md:group-hover:border-primary border-gray-400'} relative appearance-none h-4 w-4 checked:bg-primary border rounded-full focus:outline-none`}
                      />
                      <span className="flex items-center ml-0.5 w-3 h-3 text-white absolute"><IconCheck /></span>
                      <div className="flex items-center ml-1">
                        <div className={`${isTwoPaysChecked ? 'text-gray-700' : 'text-gray-500'} ml-1 text-sm whitespace-nowrap`}>
                          Dos metodos de pago
                        </div>
                      </div>
                    </div>
                ...
                <div className="">
                  <div className="flex text-sm relative">
                    <p className="font-semibold text-gray-700 ml-2 pl-1 sm:font-semibold ">{`Metodo${isTwoPaysChecked ? 's' : ''} de Pago`}</p>
                  </div>
                  <div className="flex items-start space-x-2 mb-2">
                    <label htmlFor="payMethodOne" className={`flex flex-col justify-center ${isTwoPaysChecked ? ' w-1/2' : 'w-full'}`}>
                      <div className="relative">
                        <select
                          ref={refPayMethodOne}
                          disabled={isTwoPaysChecked}
                          id="payMethodOne"
                          className={`appearance-none bg-white pl-3 py-0.5 text-sm rounded-xl w-full ring-1 ${isTwoPaysChecked ? '' : 'focus:ring-2'} focus:outline-none ring-blue-400 text-gray-800'`}
                        >
                              ...
                              <>
                                {
                                  ...
                                  && <option key={item.nombre} value={item.nombre} className="text-gray-700 capitalize">{item.nombre}</option>
                                }
                              </>
                            ))}
                    ...
                    {isTwoPaysChecked && (
                      <div className="w-1/2">
                        <label htmlFor="payCantOne" className="w-full relative flex flex-col justify-center ">
                          ...
                          <input
                            ...
                            step={0.01}
                            type="number"
                            onChange={(e) => handleChangePrice(e, setPayCantOne, setPayCantTwo, 'payCantOne', 'payCantTwo')}
                            onBlur={() => (payCantOne === ''
                              ? handleChangePrice({ target: { value: 0 } }, setPayCantOne, setPayCantTwo, 'payCantOne', 'payCantTwo')
                              : setPayCantOne(parseFloat(payCantOne).toFixed(2)))}
                            value={payCantOne}
                            className={`${errors?.payCantOne ? 'ring-red-500' : 'ring-blue-400'} bg-white pl-3 pr-3 py-0.5 text-sm rounded-xl w-full ring-1 focus:ring-2 focus:outline-none text-gray-800`}
                          />
                          <input
                            type="text"
                            className="hidden"
                  ...
                  {isTwoPaysChecked
                    && (
                      <div className="flex items-start space-x-2 mb-2">
                        <label htmlFor="payMethodTwo" className="flex flex-col justify-center w-1/2">
                          <div className="relative">
                            <select
                              ref={refPayMethodTwo}
                              id="payMethodTwo"
                            ...
                            >
                              ...
                              {Array.isArray(paymentTypes)
                                && paymentTypes.map((item) => (
                                  <>
                                    {item.nombre !== 'Efectivo' && (
                                      <option key={`${item.nombre}Two`} value={item.nombre} className="text-gray-700 capitalize">{item.nombre}</option>
                                    )}
                                  </>
                                ))}
                            </select>
                        ...
                        <div className="w-1/2">
                          <label htmlFor="payCantTwo" className="w-full relative flex flex-col justify-center ">
                            <input
                              placeholder="Ingresar Monto"
                              inputMode="decimal"
                              pattern="[0-9.]*"
                              step={0.01}
                              onChange={(e) => handleChangePrice(e, setPayCantTwo, setPayCantOne, 'payCantTwo', 'payCantOne')}
                              // onBlur={() => payCantOne === '' && handleChangePrice({ target: { value: 0 } }, setPayCantOne, setPayCantTwo, 'payCantTwo', 'payCantOne')}
                              onBlur={() => (payCantTwo === ''
                                ? handleChangePrice({ target: { value: 0 } }, setPayCantTwo, setPayCantOne, 'payCantTwo', 'payCantOne')
                                : setPayCantTwo(parseFloat(payCantTwo).toFixed(2)))}
                              value={payCantTwo}
                              type="number"
                              className={`${errors?.payCantTwo ? 'ring-red-500' : 'ring-blue-400'} bg-white pl-3 pr-3 py-0.5 text-sm rounded-xl w-full ring-1 focus:ring-2 focus:outline-none text-gray-800`}
                            />
                            <input
                              type="text"
                              className="hidden"
                            ...
                            />
                            <span className={`${errors?.payCantOne || errors?.payCantTwo ? '' : 'hidden'} pl-2 text-xs text-red-500 mt-0.5`}>
                              {errors?.payCantOne?.message || errors?.payCantTwo?.message}
                            </span>
                          </label>
                        </div>
                      </div>
                ...
                <div className="flex justify-end p-1">
                  ...
                  <div className="flex flex-col items-end text-sm text-gray-600 font-semibold ">
                    <p className="px-2.5 text-right whitespace-nowrap">{`S/ ${(priceProducts / 1.18).toFixed(2)}`}</p>
                    <p className="px-2.5 text-right whitespace-nowrap">{`S/ ${((priceProducts * 0.18) / 1.18).toFixed(2)}`}</p>
                    <p className="px-2.5 text-right whitespace-nowrap">{`S/ ${priceProducts.toFixed(2)}`}</p>
                  </div>
                </div>
